...
#include "ui_zarticontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zartiTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zartiTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zARTI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zARTI seed used to mint this zARTI is not the same as currently hold in the wallet";
            else
