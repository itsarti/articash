...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZARTIModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zartiTracker->HasMintTx(hash))) {
            //zARTI stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZARTI;
...
bool IsZARTIType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZARTI:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZARTI || type == TransactionRecord::MNReward) {
