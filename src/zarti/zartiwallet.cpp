...
CzARTIWallet::CzARTIWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zarti seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZARTISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZARTISeed_deprecated()) {
                    LogPrintf("%s: Updated zARTI seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zarti seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zarti wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzARTIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZARTICount(nCountLastUsed);
    else if (!walletdb.ReadZARTICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzARTIWallet::Lock()
{
    seedMaster = 0;
}

void CzARTIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzARTIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZARTI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzARTIWallet::LoadMintPoolFromDB()
{
...
void CzARTIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzARTIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzARTIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zartiTracker->HasPubcoinHash(pMint.first)) {
...
bool CzARTIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZARTI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zartiTracker which also adds to database
    pwalletMain->zartiTracker->Add(dMint, true);
...
void CzARTIWallet::SeedToZARTI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzARTIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzARTIWallet::GenerateDeterministicZARTI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzARTIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzARTIWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzARTIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
