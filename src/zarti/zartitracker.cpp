...
CzARTITracker::CzARTITracker(std::string strWalletFile)
{
...
CzARTITracker::~CzARTITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzARTITracker::Init()
{
...
bool CzARTITracker::Archive(CMintMeta& meta)
{
...
bool CzARTITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzARTITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzARTITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzARTITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzARTITracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzARTITracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzARTITracker::GetSerialHashes()
{
...
CAmount CzARTITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzARTITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzARTITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzARTITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzARTITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzARTITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzARTITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzARTITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzARTITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzARTITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzARTITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzARTIWallet* zARTIWallet)
{
    bool iszARTIWalletInitialized = (NULL != zARTIWallet);
    CMintMeta meta;
    ...
    if (! iszARTIWalletInitialized)
        zARTIWallet = new CzARTIWallet(strWalletFile);
    meta.isSeedCorrect = zARTIWallet->CheckSeed(dMint);
    if (! iszARTIWalletInitialized)
        delete zARTIWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzARTITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzARTITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzARTITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzARTITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzARTITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzARTITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzARTIWallet* zARTIWallet = new CzARTIWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zARTIWallet);
        }
        delete zARTIWallet;
        LogPrint("zero", "%s: added %d dzarti from DB\n", __func__, listDeterministicDB.size());
    }
