...
public:
    ...
    bool WriteZARTISeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZARTISeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZARTISeed_deprecated(uint256& seed);
    bool EraseZARTISeed();
    bool EraseZARTISeed_deprecated();

    bool WriteZARTICount(const uint32_t& nCount);
    bool ReadZARTICount(uint32_t& nCount);
