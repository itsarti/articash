...
// The zARTI block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zARTI is a hash of the serial
...
{
    //Create an output returning the zARTI that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZARTIOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zARTI output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zARTI", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZARTIOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zARTI output", __func__);
...
{
    CzARTITracker* zartiTracker = pwallet->zartiTracker.get();
    CMintMeta meta;
    if (!zartiTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zartiTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a ARTI stake is the outpoint
